    ...
    if (ImGui::Begin("Resources"))
    {
        if (ImGui::BeginTabBar("##bar"))
        {
            ...
            if (ImGui::BeginTabItem("Costumes"))
            {
                ...
                if (ImGui::BeginChild("##select-costume", {column_width, 0}))
                {
                    size_t moveSrc = 0;
                    size_t moveDst = 0;
                    auto moved = false;
                    ...
                    for (auto &costume: sprite.Costumes)
                    {
                        ...
                        auto sel = sprite.CurrentCostume == i;
                    ...
                    if (moved && moveSrc != moveDst)
                    {
                        if (moveDst > moveSrc)
                        {
                            ...
                            for (auto j = moveSrc; j < moveDst; j++)
                                sprite.Costumes[j] = std::move(sprite.Costumes[j + 1]);
                            sprite.Costumes[moveDst] = std::move(costume);
                        }
                        else
                        {
                            auto &costume = sprite.Costumes[moveSrc];
