...
static std::string read_file(const std::string &name)
{
    ...
    source.assign(std::istreambuf_iterator(stream), std::istreambuf_iterator<char>());
    return source;
}

static void check_compiled(const GLuint shader)
{
    GLint status;
...
scr::Program::Program(const std::string &vertex, const std::string &fragment)
{
    ...
    {
        const auto source = read_file(vertex);
    ...
    {
        const auto source = read_file(fragment);
        const auto c_str = source.c_str();
