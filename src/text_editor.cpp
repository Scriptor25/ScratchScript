...
void TextEditor::SetLanguageDefinition(const LanguageDefinition &aLanguageDef)
{
    ...
    for (auto &[fst, snd]: mLanguageDefinition.mTokenRegexStrings)
        mRegexList.emplace_back(
            std::regex(fst, std::regex_constants::optimize),
            snd);
...
std::string TextEditor::GetText(const Coordinates &aStart, const Coordinates &aEnd) const
{
    ...
    const auto iend = GetCharacterIndex(aEnd);
    size_t s = 0;
    ...
    while (istart < iend || lstart < lend)
    {
        ...
        if (auto &line = mLines[lstart]; istart < static_cast<int>(line.size()))
        {
...
void TextEditor::Advance(Coordinates &aCoordinates) const
{
    if (aCoordinates.mLine < static_cast<int>(mLines.size()))
    {
        ...
        {
            const auto delta = UTF8CharLength(line[cindex].mChar);
...
int TextEditor::InsertTextAt(Coordinates & /* inout */ aWhere, const char *aValue)
{
    ...
    auto cindex = GetCharacterIndex(aWhere);
    auto totalLines = 0;
    while (*aValue != '\0')
    {
        assert(!mLines.empty());
...
TextEditor::Coordinates TextEditor::ScreenPosToCoordinates(const ImVec2 &aPosition) const
{
    ...
    const auto lineNo = std::max(0, static_cast<int>(std::floor(local.y / mCharAdvance.y)));
    ...
    if (lineNo >= 0 && lineNo < static_cast<int>(mLines.size()))
    {
        ...
        {
            auto columnWidth = 0.0f;

            if (line[columnIndex].mChar == '\t')
            {
                const auto spaceSize = ImGui::GetFont()->CalcTextSizeA(ImGui::GetFontSize(), FLT_MAX, -1.0f, " ").x;
                const auto oldX = columnX;
                const auto newColumnX = (1.0f + std::floor(
                                             (1.0f + columnX) / (static_cast<float>(mTabSize) * spaceSize))) *
...
TextEditor::Coordinates TextEditor::FindWordStart(const Coordinates &aFrom) const
{
    ...
    const auto cstart = static_cast<PaletteIndex>(line[cindex].mColorIndex);
    while (cindex > 0)
    {
        const auto c = line[cindex].mChar;
        if ((c & 0xC0) != 0x80) // not UTF code sequence 10xxxxxx
        {
            ...
            if (cstart != static_cast<PaletteIndex>(line[static_cast<size_t>(cindex - 1)].mColorIndex))
                break;
        }
        --cindex;
    }
...
TextEditor::Coordinates TextEditor::FindWordEnd(const Coordinates &aFrom) const
{
    ...
    const auto prevspace = static_cast<bool>(isspace(line[cindex].mChar));
    const auto cstart = static_cast<PaletteIndex>(line[cindex].mColorIndex);
...
bool TextEditor::IsOnWordBoundary(const Coordinates &aAt) const
{
    ...
    const auto cindex = GetCharacterIndex(aAt);
    if (cindex >= static_cast<int>(line.size()))
        return true;
...
void TextEditor::RemoveLine(const int aStart, const int aEnd)
{
    ...
    ErrorMarkers etmp;
    for (auto &[fst, snd]: mErrorMarkers)
    {
        ErrorMarkers::value_type e(fst >= aStart ? fst - 1 : fst, snd);
...
void TextEditor::RemoveLine(const int aIndex)
{
    ...
    for (auto &[fst, snd]: mErrorMarkers)
    {
        ErrorMarkers::value_type e(fst > aIndex ? fst - 1 : fst, snd);
        if (e.first - 1 == aIndex)
            continue;
        etmp.insert(e);
    }
...
TextEditor::Line &TextEditor::InsertLine(const int aIndex)
{
    assert(!mReadOnly);
    ...
    for (auto &[fst, snd]: mErrorMarkers)
        etmp.insert(
            ErrorMarkers::value_type(fst >= aIndex ? fst + 1 : fst, snd));
    mErrorMarkers = std::move(etmp);
...
ImU32 TextEditor::GetGlyphColor(const Glyph &aGlyph) const
{
    ...
    auto const color = mPalette[static_cast<int>(aGlyph.mColorIndex)];
    if (aGlyph.mPreprocessor)
    {
        const auto ppcolor = mPalette[static_cast<int>(PaletteIndex::Preprocessor)];
        const auto c0 = static_cast<int>((ppcolor & 0xff) + (color & 0xff)) / 2;
        const auto c1 = static_cast<int>((ppcolor >> 8 & 0xff) + (color >> 8 & 0xff)) / 2;
        const auto c2 = static_cast<int>((ppcolor >> 16 & 0xff) + (color >> 16 & 0xff)) / 2;
        const auto c3 = static_cast<int>((ppcolor >> 24 & 0xff) + (color >> 24 & 0xff)) / 2;
...
void TextEditor::HandleKeyboardInputs()
{
    ...
    if (ImGui::IsWindowFocused())
    {
        ...
        if ((!IsReadOnly() && ctrl && !shift && !alt && ImGui::IsKeyPressed(ImGuiKey_Z)) ||
            (!IsReadOnly() && !ctrl && !shift && alt && ImGui::IsKeyPressed(ImGuiKey_Backspace)))
            Undo();
            ...
            mOverwrite ^= true;
                 ...
                 (ctrl && !shift && !alt && ImGui::IsKeyPressed(ImGuiKey_C)))
            Copy();
        else if (
            (!IsReadOnly() && !ctrl && shift && !alt && ImGui::IsKeyPressed(ImGuiKey_Insert)) ||
            (!IsReadOnly() && ctrl && !shift && !alt && ImGui::IsKeyPressed(ImGuiKey_V)))
            Paste();
        else if ((ctrl && !shift && !alt && ImGui::IsKeyPressed(ImGuiKey_X)) ||
                 (!ctrl && shift && !alt && ImGui::IsKeyPressed(ImGuiKey_Delete)))
            Cut();
...
void TextEditor::Render()
{
    ...
    const auto fontSize = ImGui::GetFont()->CalcTextSizeA(
        ImGui::GetFontSize(),
        FLT_MAX,
        -1.0f,
        "#",
    ...
    for (auto i = 0; i < static_cast<int>(PaletteIndex::Max); ++i)
    {
    ...
    auto lineMax = std::max(
        ...
        std::min(
            ...
            lineNo + static_cast<int>(std::floor((scrollY + contentSize.y) / mCharAdvance.y))));
    ...
    if (!mLines.empty())
    {
        auto spaceSize = ImGui::GetFont()->CalcTextSizeA(
            ImGui::GetFontSize(),
            FLT_MAX,
            -1.0f,
            " ",
        ...
        while (lineNo <= lineMax)
        {
            auto lineStartScreenPos = ImVec2(
                cursorScreenPos.x,
                cursorScreenPos.y + static_cast<float>(lineNo) * mCharAdvance.y);
            ...
            if (mBreakpoints.contains(lineNo + 1))
            {
                ...
                drawList->AddRectFilled(start, end, mPalette[static_cast<int>(PaletteIndex::Breakpoint)]);
            }
            ...
            drawList->AddText(
                ...
                mPalette[static_cast<int>(PaletteIndex::LineNumber)],
                buf);
            ...
            if (mState.mCursorPosition.mLine == lineNo)
            {
                ...
                if (!HasSelection())
                {
                    ...
                    drawList->AddRectFilled(
                        ...
                        mPalette[static_cast<int>(focused
            ...
            for (auto i = 0; i < line.size();)
            {
                ...
                if (glyph.mChar == '\t')
                {
                    ...
                    bufferOffset.x = (1.0f + std::floor(
                                          (1.0f + bufferOffset.x) / (static_cast<float>(mTabSize) * spaceSize))) *
                                     (static_cast<float>(mTabSize) * spaceSize);
                    ++i;
...
void TextEditor::Render(const char *aTitle, const ImVec2 &aSize, const bool aBorder)
{
    mWithinRender = true;
    mTextChanged = false;
    mCursorPositionChanged = false;
    ...
    ImGui::PushStyleColor(
        ImGuiCol_ChildBg,
        ImGui::ColorConvertU32ToFloat4(mPalette[static_cast<int>(PaletteIndex::Background)]));
...
void TextEditor::EnterCharacter(ImWchar aChar, bool aShift)
{
    ...
    if (HasSelection())
    {
        if (aChar == '\t' && mState.mSelectionStart.mLine != mState.mSelectionEnd.mLine)
        {
            ...
            for (auto i = start.mLine; i <= end.mLine; i++)
            {
                auto &line = mLines[i];
                if (aShift)
                {
                    if (!line.empty())
                    {
                        ...
                        {
                            for (auto j = 0; j < mTabSize && !line.empty() && line.front().mChar == ' '; j++)
                            {
                                line.erase(line.begin());
                                modified = true;
                            }
    ...
    if (aChar == '\n')
    {
        ...
        const auto whitespaceSize = newLine.size();
        auto cindex = GetCharacterIndex(coord);
        newLine.insert(newLine.end(), line.begin() + cindex, line.end());
        line.erase(line.begin() + cindex, line.begin() + static_cast<long long>(line.size()));
        SetCursorPosition(
            Coordinates(coord.mLine + 1, GetCharacterColumn(coord.mLine + 1, static_cast<int>(whitespaceSize))));
        u.mAdded = static_cast<char>(aChar);
    }
    else
    {
        char buf[7];
        auto e = ImTextCharToUtf8(buf, 7, aChar);
        if (e > 0)
        {
            buf[e] = '\0';
            auto &line = mLines[coord.mLine];
    ...
    switch (aMode)
    {
        ...
        case SelectionMode::Word:
        {
            ...
            if (!IsOnWordBoundary(mState.mSelectionEnd))
                mState.mSelectionEnd = FindWordEnd(
                    FindWordStart(mState.mSelectionEnd));
            break;
        }
...
void TextEditor::InsertText(const char *aValue)
{
    ...
    auto totalLines = pos.mLine - start.mLine;
...
void TextEditor::MoveDown(const int aAmount, const bool aSelect)
{
    ...
    mState.mCursorPosition.mLine = std::max(
        0,
        std::min(static_cast<int>(mLines.size()) - 1, mState.mCursorPosition.mLine + aAmount));
...
void TextEditor::Delete()
{
    ...
    else
    {
        ...
        {
            const auto cindex = GetCharacterIndex(pos);
            u.mRemovedStart = u.mRemovedEnd = GetActualCursorCoordinates();
            ...
            for (auto &[fst, snd]: mErrorMarkers)
                etmp.insert(
                    ErrorMarkers::value_type(
                        fst - 1 == mState.mCursorPosition.mLine ? fst - 1 : fst,
                        snd));
            mErrorMarkers = std::move(etmp);
...
void TextEditor::Colorize(const int aFromLine, const int aCount)
{
    const auto toLine = aCount == -1
                            ? static_cast<int>(mLines.size())
                            : std::min(static_cast<int>(mLines.size()), aFromLine + aCount);
...
void TextEditor::ColorizeRange(const int aFromLine, const int aToLine)
{
    ...
    const auto endLine = std::max(0, std::min(static_cast<int>(mLines.size()), aToLine));
    for (auto i = aFromLine; i < endLine; ++i)
    {
        auto &line = mLines[i];
        ...
        const auto bufferEnd = bufferBegin + buffer.size();
        ...
        for (auto first = bufferBegin; first != last;)
        {
            ...
            auto token_color = PaletteIndex::Default;

            auto hasTokenizeResult = false;
            ...
            {
                for (auto &[fst, snd]: mRegexList)
                {
                    if (std::regex_search(first, last, results, fst, std::regex_constants::match_continuous))
                    {
                        hasTokenizeResult = true;
...
void TextEditor::ColorizeInternal()
{
    ...
    if (mCheckComments)
    {
        ...
        while (currentLine < endLine || currentIndex < endIndex)
        {
            ...
            if (!line.empty())
            {
                ...
                auto inComment = commentStartLine < currentLine ||
                ...
                if (withinString)
                {
                    ...
                    if (c == '\"')
                    {
                        ...
                        {
                            currentIndex += 1;
                            if (currentIndex < static_cast<int>(line.
                                    size()))
                                line[currentIndex].mMultiLineComment = inComment;
                        }
                        else
                    ...
                    else
                    {
                        ...
                        if (!singleStartStr.empty() &&
                            ...
                            equals(
                                ...
                                singleStartStr.end(),
                                from,
                                from + static_cast<long long>(singleStartStr.size()),
                                pred))
                        {
                            withinSingleLineComment = true;
                                 ...
                                 equals(
                                     ...
                                     startStr.end(),
                                     from,
                                     from + static_cast<long long>(startStr.size()),
                                     pred))
                        {
                        ...
                        inComment = inComment = commentStartLine < currentLine || (
                                                    commentStartLine == currentLine && commentStartIndex <=
                                                    currentIndex);
                        ...
                        if (currentIndex + 1 >= static_cast<int>(endStr.size()) &&
                            equals(
                                endStr.begin(),
                                endStr.end(),
                                from + 1 - static_cast<long long>(endStr.size()),
                                from + 1,
                                pred))
                        {
    ...
    {
        const auto increment = mLanguageDefinition.mTokenize == nullptr ? 10 : 10000;
        const auto to = std::min(mColorRangeMin + increment, mColorRangeMax);
...
float TextEditor::TextDistanceToLineStart(const Coordinates &aFrom) const
{
    ...
    auto distance = 0.0f;
    const auto spaceSize = ImGui::GetFont()->CalcTextSizeA(
        ImGui::GetFontSize(),
        FLT_MAX,
        -1.0f,
        " ",
        nullptr,
        nullptr).x;
    const auto colIndex = GetCharacterIndex(aFrom);
    for (size_t it = 0u; it < line.size() && it < colIndex;)
    {
        ...
        else
        {
            ...
            char tempCString[7];
            auto i = 0;
            for (; i < 6 && d-- > 0 && it < static_cast<int>(line.size());
                   i++, it++)
...
void TextEditor::EnsureCursorVisible()
{
    ...
    const auto top = 1 + static_cast<int>(std::ceil(scrollY / mCharAdvance.y));
    const auto bottom = static_cast<int>(std::ceil((scrollY + height) / mCharAdvance.y));

    const auto left = static_cast<int>(std::ceil(scrollX / mCharAdvance.x));
    const auto right = static_cast<int>(std::ceil((scrollX + width) / mCharAdvance.x));
    ...
    if (pos.mLine > bottom - 4)
        ImGui::SetScrollY(
            std::max(0.0f, static_cast<float>(pos.mLine + 4) * mCharAdvance.y - height));
    if (len + mTextStart < static_cast<float>(left) + 4)
        ImGui::SetScrollX(std::max(0.0f, len + mTextStart - 4));
    if (len + mTextStart > static_cast<float>(right) - 4)
        ImGui::SetScrollX(
            std::max(0.0f, len + mTextStart + 4 - width));
}
...
static bool TokenizeCStyleNumber(const char *in_begin, const char *in_end, const char *&out_begin, const char *&out_end)
{
    ...
    const auto startsWithNumber = *p >= '0' && *p <= '9';
...
const TextEditor::LanguageDefinition &TextEditor::LanguageDefinition::CPlusPlus()
{
    ...
    if (!inited)
    {
        static constexpr const char *cppKeywords[] = {
            "alignas",
            "alignof",
            "and",
            "and_eq",
